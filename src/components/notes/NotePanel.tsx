...
const NotePanel: React.FC = () => {
  ...
  return (
    <div 
      className={`absolute top - 0 h - full flex flex - col bg - [#1e1b17] border border - [#3f362b] z - 10 transition - [width] duration - 0 overflow - hidden shadow - 2xl ${
    edgePosition === 'right' ? 'right-full' : 'left-full'
} `}
        ...
        <div className="flex gap-2 overflow-x-auto custom-scrollbar">
          {tabs.map((tab) => (
            <button 
              ...
              className={`px - 5 py - 2.5 text - sm font - medium rounded - t - lg transition - colors flex items - center gap - 2 whitespace - nowrap cursor - pointer ${
    tab.active
